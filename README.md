...
__Note__
By default email is not verified, so you need to verify the email with first
...
```
__Note__
In case of user already exists with provided `email`, `username`, or `phone`, 
this message can change accordingly with status `400` bad request.
...
## Extra step in case 2FA enabled
...
__Note__: In case of validation, the response message can be changed i.e. `Incorrect TOTP`.
